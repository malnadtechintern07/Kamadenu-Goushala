...
$stmt = $pdo->prepare("SELECT p.*, c.name as category_name, wn.phone_number as wa_phone_dir FROM products p JOIN product_categories c ON p.category_id = c.id LEFT JOIN whatsapp_numbers wn ON p.whatsapp_number_id = wn.id WHERE p.id = ?");
$stmt->execute([$id]);
$product = $stmt->fetch();
        ...
        <div class="row g-5">
            ...
            <div class="col-lg-6">
                <div class="kamadenu-card p-4 p-md-5">
                    ...
                    <?php 
                        $pname = __td($product, 'name');
                        $pdesc = __td($product, 'description');
                    ?>
                    <h2 class="font-heading mb-2"><?php echo e($pname); ?></h2>

                    <div class="fs-2 fw-bold text-dark font-mono mb-3">₹<?php echo number_format($product['price'], 2); ?></div>

                    <p class="lead text-secondary mb-4"><?php echo e($pdesc); ?></p>
