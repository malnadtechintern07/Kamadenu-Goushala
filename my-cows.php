...
if (!is_user_logged_in()) { header("Location: /Kamadhenu-goushala/login.php?redirect=" . urlencode('/Kamadhenu-goushala/my-cows.php') . "&msg=login_required"); exit; }
    ...
    <div class="container">
        <?php if (empty($sponsored_cows)): ?>
            <div class="kamadenu-card p-5 text-center">
                ...
                <a href="/Kamadhenu-goushala/adopt.php" class="btn btn-warning font-ui fw-bold px-4 py-2 mt-2">Sponsor a Cow</a>
            </div>
        <?php else: ?>
            <div class="row g-4">
                <?php foreach ($sponsored_cows as $sc): ?>
                    <div class="col-md-6">
                        <div class="kamadenu-card p-4 d-flex align-items-center gap-4">
                            <img src="/Kamadhenu-goushala/<?php echo e($sc['photo']); ?>" class="rounded-circle shadow" style="width: 90px; height: 90px; object-fit: cover; flex-shrink: 0;" onerror="this.src='https://images.unsplash.com/photo-1546445317-29f4545f9d52?auto=format&fit=crop&w=100&q=80'">
                            <div>
                                ...
                                <a href="/Kamadhenu-goushala/cow-detail.php?id=<?php echo $sc['cow_id']; ?>" class="btn btn-sm btn-kamadenu-primary font-ui fw-bold">View Cattle Profile &rarr;</a>
                            </div>
                        </div>
                    </div>
                <?php endforeach; ?>
